 ...
 *      strategy.</li>
 *   <li>A hyphen "{@code -}" will emit a {@link HclTokenType#HEREDOC_INDENT_MARKER}.</li>
 *   <li>An {@code ID_START} character will emit an {@link HclTokenType#IDENTIFIER} and cache this
 *      value.</li>
 *   <li>A line feed {@code LF} or carriage return and line feed {@code CRLF} will emit a
 *      {@link HclTokenType#NEW_LINE} token. If there is an identifier already cached, this
 *      identifier will be passed to a new {@link HclHeredocLexerStrategy}. This header lexer
 *      strategy will be popped and the new strategy will be pushed. If no identifier was
 *      discovered, we are in an erroneous state so we cannot do anything else. In this case we will
 *      pop the current mode only. The parser consuming this stream of tokens should decide how to
 ...
 * {@link HclTokenType#HEREDOC_ANCHOR} was produced by the lexer strategy that initialized this
 ...
 */
public final class HclHeredocHeaderLexerStrategy extends HclCommonLexerStrategyBase {
   ...
   */
  public HclHeredocHeaderLexerStrategy(HclDefaultLexer context) {
    super(context);
    identifier = null;
  }
  ...
  public HclToken nextToken() throws HclProcessingException {
    ...
    if (next == '-') {
      return newToken(HclTokenType.HEREDOC_INDENT_MARKER, 1);
    }
    ...
    if (isNewLineStart(next)) {
      ...
      if (identifier != null) {
        context.pushStrategy(new HclHeredocLexerStrategy(context, identifier));
        return newLine;
      }
    }
