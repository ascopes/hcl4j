...
/**
 * An {@link HclCharSource} that wraps a given {@link InputStream} internally, buffering it and
 ...
 * @since 0.0.1
 */
public final class HclCharInputStream implements HclCharSource {
  ...
  @Nullable
  private HclLocation cachedLocation;
   ...
   */
  public HclCharInputStream(@Nullable String name, InputStream inputStream) throws IOException {
  ...
  @Override
  public void advance(int count) throws HclStreamException {
    if (count <= 0) {
    ...
    try {
      ...
      throw new HclStreamException("Call to CharSource#advance failed due to an IO error", ex);
    }
  }

  @Override
  public void close() throws HclStreamException {
    try {
      reader.close();
    } catch (IOException ex) {
      throw new HclStreamException("Call to CharSource#close failed due to an IO error", ex);
    }
  }
  ...
  public HclLocation location() {
    ...
    return cachedLocation == null
        ? (cachedLocation = new HclLocation(position, line, column))
        : cachedLocation;
  }
  ...
  public int peek(int offset) throws HclStreamException {
    if (offset < 0) {
    ...
    try {
      ...
      throw new HclStreamException("Call to CharSource#peek failed due to an IO error", ex);
    }
  }

  @Override
  public int read() throws HclStreamException {
    try {
      var next = reader.read();
      processNextChar(next);
      return next;
    } catch (IOException ex) {
      throw new HclStreamException("Call to CharSource#read failed due to an IO error", ex);
    }
  }

  @Override
  public CharSequence readString(int count) throws HclStreamException {
    if (count < 0) {
    ...
    try {
      ...
      throw new HclStreamException("Call to CharSource#readString failed due to an IO error", ex);
    }
  ...
  @Override
  public boolean startsWith(CharSequence match) throws HclStreamException {
    ...
    try {
      ...
      throw new HclStreamException("Call to CharSource#startsWith failed due to an IO error", ex);
    }
  }
